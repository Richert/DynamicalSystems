...
# model parameters
node_vars = {"tau": 1.0, "J": 20.0, "eta": -2.0, "Delta": 0.5, "tau_s": 0.2, "alpha": 0.0, "tau_a": 10.0,
             "kappa": 0.0, "tau_u": 500.0, "a_max": 1.0, "a_min": 0.5}
...
for i, p1_val in enumerate(p1_vals):
    ...
    c2_sols, c2_cont = ode.run(starting_point=f'UZ{i+1}', ICP=eta_idx, name=f'eta:{i+1}', DSMAX=0.01,
                               origin=c1_cont, UZR={}, STOP=[], NPR=2, RL1=10.0, RL0=-10.0, bidirectional=True)

    try:
        ode.run(starting_point="HB1", ICP=[eta_idx, 11], name=f"eta:{i+1}:lc:1", origin=c2_cont, ISW=-1, IPS=2, NMX=8000,
                DSMAX=0.04, NCOL=6, NTST=200, STOP=["LP4", "BP2"], EPSL=1e-8, EPSU=1e-8, EPSS=1e-5, NPR=10)
    except KeyError:
        pass
    try:
...
try:
    ode.run(starting_point='LP1', ICP=[p1_idx, eta_idx], name=f'{p1}/eta:lp1', origin=f"eta:{p1_val_idx+1}", NMX=4000, DSMAX=0.1,
            NPR=10, RL1=2.0, RL0=0.0, bidirectional=True, ILP=0, IPS=1, ISW=2, NTST=400)
    ode.run(starting_point='LP2', ICP=[p1_idx, eta_idx], name=f'{p1}/eta:lp2', origin=f"eta:{p1_val_idx+1}", NMX=4000, DSMAX=0.1,
            NPR=10, RL1=2.0, RL0=0.0, bidirectional=True, ILP=0, IPS=1, ISW=2, NTST=400)
except KeyError:
    fold_bifurcations = False
try:
    ode.run(starting_point='HB1', ICP=[p1_idx, eta_idx], name=f'{p1}/eta:hb1', origin=f"eta:{p1_val_idx+1}", NMX=4000, DSMAX=0.1,
            NPR=10, RL1=2.0, RL0=0.0, bidirectional=True, ILP=0, IPS=1, ISW=2, NTST=400, STOP=["BP2"])
except KeyError:
    hopf_bifurcation_1 = False
try:
    ode.run(starting_point='HB2', ICP=[p1_idx, eta_idx], name=f'{p1}/eta:hb2', origin=f"eta:{p1_val_idx+1}", NMX=4000, DSMAX=0.1,
            NPR=10, RL1=2.0, RL0=0.0, bidirectional=True, ILP=0, IPS=1, ISW=2, NTST=400, STOP=["BP2"])
except KeyError:
