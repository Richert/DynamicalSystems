...
# model parameters
node_vars = {"tau": 1.0, "J": 20.0, "eta": -4.0, "Delta": 1.0, "tau_s": 0.2, "alpha": 0.1, "tau_a": 10.0,
             "kappa": 0.0, "tau_u": 100.0, "A0": 0.5}
...
for i, p1_val in enumerate(p1_vals):
    ...
    c2_sols, c2_cont = ode.run(starting_point=f'UZ{i+1}', ICP=eta_idx, name=f'eta:{i+1}', DSMAX=0.01,
                               origin=c1_cont, UZR={}, STOP=[], NPR=5, RL1=10.0, RL0=-10.0, bidirectional=True,
                               variables=["U(1)"])

    try:
        ode.run(starting_point="HB1", ICP=[eta_idx, 11], name=f"eta:{i+1}:lc:1", origin=c2_cont, ISW=-1, IPS=2, NMX=6000,
                DSMAX=0.04, NCOL=6, NTST=200, STOP=["LP6", "BP2"], EPSL=1e-7, EPSU=1e-7, EPSS=1e-4, NPR=10,
                variables=["U(1)"])
    except KeyError:
        pass
    try:
...
try:
    ode.run(starting_point='LP1', ICP=[p1_idx, eta_idx], name=f'{p1}/eta:lp1', origin=f"eta:{p1_val_idx+1}", NMX=4000,
            DSMAX=0.1, NPR=10, RL1=2.0, RL0=0.0, bidirectional=True, ILP=0, IPS=1, ISW=2, NTST=400,
    ...
    ode.run(starting_point='LP2', ICP=[p1_idx, eta_idx], name=f'{p1}/eta:lp2', origin=f"eta:{p1_val_idx+1}",
            NMX=4000, DSMAX=0.1, NPR=10, RL1=2.0, RL0=0.0, bidirectional=True, ILP=0, IPS=1, ISW=2, NTST=400,
            variables=["U(1)"], get_stability=False)
except KeyError:
...
try:
    ode.run(starting_point='PD1', ICP=[p1_idx, eta_idx], name=f'{p1}/eta:pd1', origin=f"eta:{p1_val_idx + 1}:lc:2",
            NMX=4000, DSMAX=0.1, NPR=10, RL1=2.0, RL0=0.0, bidirectional=True, ILP=0, IPS=2, ISW=2, NTST=400,
...
if fold_bifurcations:
    ode.run(starting_point='LP1', ICP=[p2_idx, eta_idx], name=f'{p2}/eta:lp1', origin=f"eta:{p1_val_idx+1}",
            NMX=4000, DSMAX=0.01, NPR=10, RL1=p2_max, RL0=p2_min, bidirectional=True, ILP=0, IPS=1, ISW=2, NTST=400,
    ...
    ode.run(starting_point='LP2', ICP=[p2_idx, eta_idx], name=f'{p2}/eta:lp2', origin=f"eta:{p1_val_idx+1}", NMX=4000,
            DSMAX=0.01, NPR=10, RL1=p2_max, RL0=p2_min, bidirectional=True, ILP=0, IPS=1, ISW=2, NTST=400, STOP=["BP2"],
            variables=["U(1)"], get_stability=False)
...
if pd_bifurcation:
    ode.run(starting_point='PD1', ICP=[p2_idx, eta_idx], name=f'{p2}/eta:pd1', origin=f"eta:{p1_val_idx+1}:lc:2",
            NMX=4000, DSMAX=0.01, NPR=10, RL1=p2_max, RL0=p2_min, bidirectional=True, ILP=0, IPS=2, ISW=2, NTST=400,
            get_stability=False, variables=["U(1)"])
if tr_bifurcation:
...
if fold_bifurcations:
    ode.run(starting_point='LP1', ICP=[p2_idx, p1_idx], name=f'{p2}/{p1}:lp1', origin=f"eta:{p1_val_idx+1}", NMX=4000,
            DSMAX=0.01, NPR=10, RL1=p2_max, RL0=p2_min, bidirectional=True, ILP=0, IPS=1, ISW=2, NTST=400,
            variables=["U(1)"], get_stability=False)
