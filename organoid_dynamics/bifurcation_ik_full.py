...
"""
Bifurcation analysis of an Izhikevich mean-field model with synaptic short-term plasticity and AMPA+NMDA synapses.

To run this code, you need Python >= 3.8 with PyRates (https://github.com/pyrates-neuroscience/PyRates) and 
...
t_sols, t_cont = ode.run(c='ivp', name='t', DS=1e-4, DSMIN=1e-10, EPSL=1e-06, NPR=1000, NPAR=n_params, NDIM=n_dim,
                         EPSU=1e-06, EPSS=1e-05, DSMAX=0.1, NMX=90000, UZR={14: 6000.0}, STOP={'UZ1'})
...
try:
    ...
    ode.run(starting_point='LP2', ICP=[p1_idx, eta_idx], name=f'{p1}/eta:lp2', origin=f"eta:{p1_val_idx+1}",
            NMX=NMX, DSMAX=p1_max_step, NPR=10, RL1=p1_max, RL0=p1_min, bidirectional=True, ILP=0, IPS=1, ISW=2,
...
try:
    ode.run(starting_point='HB2', ICP=[p1_idx, eta_idx], name=f'{p1}/eta:hb2', origin=f"eta:{p1_val_idx+1}",
            NMX=NMX, DSMAX=p1_max_step, NPR=10, RL1=p1_max, RL0=p1_min, bidirectional=True, ILP=0, IPS=1, ISW=2, NTST=400,
            variables=["U(1)"], get_stability=False)
except KeyError:
    hopf_bifurcation_2 = False
try:
    ode.run(starting_point='PD1', ICP=[p1_idx, eta_idx], name=f'{p1}/eta:pd1', origin=f"eta:{p1_val_idx + 1}:lc:1",
            NMX=NMX, DSMAX=p1_max_step, NPR=10, RL1=p1_max, RL0=p1_min, bidirectional=True, ILP=0, IPS=2, ISW=2, NTST=400,
...
if fold_bifurcations:
    ode.run(starting_point='LP1', ICP=[p2_idx, eta_idx], name=f'{p2}/eta:lp1', origin=f"eta:{p1_val_idx+1}",
            NMX=NMX, DSMAX=p2_max_step, NPR=10, RL1=p2_max, RL0=p2_min, bidirectional=True, ILP=0, IPS=1, ISW=2, NTST=400,
...
if pd_bifurcation:
    ode.run(starting_point='PD1', ICP=[p2_idx, eta_idx], name=f'{p2}/eta:pd1', origin=f"eta:{p1_val_idx+1}:lc:1",
            NMX=NMX, DSMAX=p2_max_step, NPR=10, RL1=p2_max, RL0=p2_min, bidirectional=True, ILP=0, IPS=2, ISW=2, NTST=400,
            get_stability=False, variables=["U(1)"])
if tr_bifurcation:
...
# # 2D continuation III
# if fold_bifurcations:
#     ode.run(starting_point='LP1', ICP=[p2_idx, p1_idx], name=f'{p2}/{p1}:lp1', origin=f"eta:{p1_val_idx+1}", NMX=NMX,
#             DSMAX=p2_max_step, NPR=10, RL1=p2_max, RL0=p2_min, bidirectional=True, ILP=0, IPS=1, ISW=2, NTST=400,
#             variables=["U(1)"], get_stability=False)
#     ode.run(starting_point='LP2', ICP=[p2_idx, p1_idx], name=f'{p2}/{p1}:lp2', origin=f"eta:{p1_val_idx+1}", NMX=NMX,
#             DSMAX=p2_max_step, NPR=10, RL1=p2_max, RL0=p2_min, bidirectional=True, ILP=0, IPS=1, ISW=2, NTST=400,
#             variables=["U(1)"], get_stability=False)
# if hopf_bifurcation_1:
#     ode.run(starting_point='HB1', ICP=[p2_idx, p1_idx], name=f'{p2}/{p1}:hb1', origin=f"eta:{p1_val_idx+1}", NMX=NMX,
#             DSMAX=p2_max_step, NPR=10, RL1=p2_max, RL0=p2_min, bidirectional=True, ILP=0, IPS=1, ISW=2, NTST=400,
#             variables=["U(1)"], get_stability=False)
# if hopf_bifurcation_2:
#     ode.run(starting_point='HB2', ICP=[p2_idx, p1_idx], name=f'{p2}/{p1}:hb2', origin=f"eta:{p1_val_idx+1}", NMX=NMX,
#             DSMAX=p2_max_step, NPR=10, RL1=p2_max, RL0=p2_min, bidirectional=True, ILP=0, IPS=1, ISW=2, NTST=400,
#             variables=["U(1)"], get_stability=False)
# if pd_bifurcation:
...
for idx1, key1, idx2, key2 in zip([p1_idx, p2_idx], [p1, p2], [eta_idx, eta_idx], ["eta", "eta"]):
    fig, ax = plt.subplots(figsize=(12, 4))
    if fold_bifurcations:
        for lp in [1, 2]:
            try:
