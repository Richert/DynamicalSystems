...
# run the recurrence analysis
#############################

delays = [2, 4, 8, 16, 32]
embeddings = []
X = np.array([res_mf["r"].values]) * 1e3
for d in delays:
    rec = RecurrencePlot(time_delay=d, threshold=0.5)
    embeddings.append(rec.transform(X))
...
fig, axes = plt.subplots(ncols=len(delays), figsize=(12, 3))
for i, (delay, embedding) in enumerate(zip(delays, embeddings)):
    ax = axes[i]
    ax.imshow(embedding[0], cmap='binary', origin='lower', aspect="auto")
