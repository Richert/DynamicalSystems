...
for d in "${deltas[@]}"; do
  for g in "${gs[@]}"; do
    for IDX in $(seq 0 $range_end); do
      ...
      srun --ntasks=1 --nodes=1 --mem=8G --time=00:30:00 --cpus-per-task=16 --job-name="dim_exc" \
      --output="out/dim_exc_$counter.out" --error="err/dim_exc_$counter.err" --partition="highcpu" --exclusive -c 1 \
