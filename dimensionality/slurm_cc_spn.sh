  ...
  for d in "${deltas[@]}"; do
    for g in "${gs[@]}"; do
      for IDX in $(seq 0 $range_end); do
        ...
        srun --ntasks=1 --nodes=1 --mem=8G --time=01:00:00 --cpus-per-task=16 --job-name="cc_spn" \
        --output="out/cc_spn_$counter.out" --error="err/cc_spn_$counter.err" --partition="highcpu" --exclusive -c 1 \
