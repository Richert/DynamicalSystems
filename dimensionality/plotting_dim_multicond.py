...
for i, c in enumerate(condition):
    ...
    scatterplot(df, x="s_norm", y="dim_ss", hue="Delta", ax=ax, s=markersize)
    ax.set_title(c.split("_")[0])
    ax = fig.add_subplot(grid[1, i])
    scatterplot(df, x="s_norm", y="dim_ss", hue="g", ax=ax, s=markersize)
...
for i, c in enumerate(condition):
    df_tmp = df.loc[df["cond"] == c, :]
    ax = fig.add_subplot(grid[0, i])
    scatterplot(df, x="s_norm", y="dim_ir", hue="Delta", ax=ax, s=markersize)
    ax.set_title(c.split("_")[0])
    ax = fig.add_subplot(grid[1, i])
    scatterplot(df, x="s_norm", y="dim_ir", hue="g", ax=ax, s=markersize)
    ax.set_xlabel("std(r)/mean(r)")
...
for i, c in enumerate(condition):
    df_tmp = df.loc[df["cond"] == c, :]
    ax = fig.add_subplot(grid[0, i])
    scatterplot(df, x="ir_tau", y="dim_ir", hue="Delta", ax=ax, s=markersize)
    ax.set_title(c.split("_")[0])
    ax = fig.add_subplot(grid[1, i])
    scatterplot(df, x="ir_tau", y="dim_ir", hue="g", ax=ax, s=markersize)
