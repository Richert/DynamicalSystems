  ...
  for d in "${deltas[@]}"; do
    for g in "${gs[@]}"; do
      for IDX in $(seq 0 $range_end); do
        ...
        srun --ntasks=1 --nodes=1 --mem=8G --time=00:45:00 --cpus-per-task=1 --job-name="cc_inh" \
        --output="out/cc_inh_$counter.out" --error="err/cc_inh_$counter.err" --partition="gpu" --exclusive -c 1 \
