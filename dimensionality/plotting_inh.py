...
for i, (y, ylabel) in enumerate(zip(["s_mean", "s_norm"], [r"$\bar s / \tau_s$", "$D(s)$"])):
    for j, p in enumerate(ivs):
        ...
        if i == 0:
            ax.set_title(rf"$s_e = {np.round(p, decimals=1)}$ pA")
            ax.set_xlabel("")
        else:
...
fig.canvas.draw()
plt.savefig(f'{path}/figures/inh_firing_rates.pdf')
...
fig.set_constrained_layout_pads(w_pad=0.03, h_pad=0.01, hspace=0., wspace=0.)
fig.canvas.draw()
plt.savefig(f'{path}/figures/inh_dimensionality.pdf')
...
fig.set_constrained_layout_pads(w_pad=0.03, h_pad=0.01, hspace=0., wspace=0.)
fig.canvas.draw()
plt.savefig(f'{path}/figures/inh_dimensionality_ss.pdf')
...
fig.set_constrained_layout_pads(w_pad=0.03, h_pad=0.01, hspace=0., wspace=0.)
fig.canvas.draw()
plt.savefig(f'{path}/figures/inh_dimensionality_ir.pdf')
...
fig.set_constrained_layout_pads(w_pad=0.03, h_pad=0.01, hspace=0., wspace=0.)
fig.canvas.draw()
plt.savefig(f'{path}/figures/inh_dimensionality_tau.pdf')
