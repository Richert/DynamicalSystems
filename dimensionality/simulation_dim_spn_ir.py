...
# fit exponential to envelope of impulse response
ir = s_mean[int(stop/dts):]
ir = ir - ir[-1]
ir = ir / np.max(ir)
tau, ir_env = impulse_response_tau(ir, tau=1.0, bounds=(1e-1, 1e3))
...
_, ax = plt.subplots(figsize=(12, 4))
ax.plot(ir, label="IR")
ax.plot(ir_env, label="Env")
ax.legend()
ax.set_xlabel("steps")
ax.set_ylabel("r")
ax.set_title(f"Decay Time Constant = {tau*dts} ms")
plt.tight_layout()
