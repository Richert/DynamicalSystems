...
inp[:, :N_e] += poisson.rvs(mu=s_e, size=(inp.shape[0], N_e))
inp[:, N_e:] += poisson.rvs(mu=s_i, size=(inp.shape[0], N_i))
inp[int((cutoff + start)/dt):int((cutoff + stop)/dt), :N_e] += poisson.rvs(mu=amp, size=(int((stop-start)/dt), N_e))
...
net.add_diffeq_node("eic", eic, input_var="g_e_in", output_var="s", spike_var="spike", reset_var="v",
                    to_file=True, op="ik_op", spike_reset=v_reset, spike_threshold=v_spike, clear=False, N=N)
...
ax.set_xlabel("steps")
ax.set_ylabel("r (Hz)")
ax.set_title(f"Dim = {dim_ir}, IR time constant: tau = {p[2]} ms")
plt.tight_layout()
