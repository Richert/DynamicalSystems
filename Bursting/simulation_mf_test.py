...
# initialize model
ik = CircuitTemplate.from_yaml("config/mf/ik_test")

# update parameters
node_vars = {'C': C, 'k': k, 'v_r': v_r, 'v_t': v_t, 'Delta': Delta, 'kappa': kappa, 'tau_u': tau_u, 'b': b,
             'tau_s': tau_s, 'g': g, 'E_r': E_r, 'tau_x': tau_x, 'eta': eta}
ik.update_var(node_vars={f"p/test_op/{key}": val for key, val in node_vars.items()})

# run simulation
res = ik.run(simulation_time=T, step_size=dt, sampling_step_size=dts, cutoff=cutoff, solver='euler',
             outputs={'s': 'p/test_op/s', 'u': 'p/test_op/u'},
             inputs={'p/test_op/I_ext': inp}, decorator=nb.njit, fastmath=True, float_precision="float64")
