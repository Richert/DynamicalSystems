...
dts = 1e-1
cutoff = 1000.0
inp = np.zeros((int(T/dt), 1)) + (-30.0 if cond == "low_kappa" else 4.0)
# inp[:int(200.0/dt)] -= 10.0
inp[int(2000/dt):int(4000/dt), 0] += (35.0 if cond == "low_kappa" else 6.0)
...
pickle.dump({"results": res, "params": node_vars}, open(f"results/snn_bs_global{file_num}.pkl", "wb"))
