...
a_min= -1.0
a_max = 1.0
bins = np.round(np.linspace(a_min, a_max, num=a_bins+1), decimals=1)
epsilon = 1e-15
gap_window = 5
window_size = 10
norm_var = False
...
for drug in drugs:
    ...
    for dose in ["Vehicle", "LowDose", "HighDose"]:
        for file in os.listdir(f"{path}/{drug}/{dose}"):
            ...
            acceleration = np.diff(smoothed_speed)

            # get speed histogram
            acc_hist = np.histogram(acceleration, bins=bins)[0]
            acc_hist = acc_hist / np.sum(acc_hist)

            # calculate result variables and save data
            for bin in range(a_bins):

                # bin-based analyses
                try:

                    # find time windows where mouse runs at particular velocity
                    idx = (bins[bin] <= acceleration) & (acceleration < bins[bin + 1])
                    a_indices = np.argwhere(idx == True).squeeze()
                    idx_diff = np.diff(a_indices)
                ...
                if np.isfinite(pr_b):
                    ...
                    res["v"].append(np.mean(smoothed_speed[:-1][idx]))
                    res["a"].append(np.round((bins[bin] + bins[bin + 1]) / 2, decimals=1))
                    res["p(a)"].append(acc_hist[bin])
...
df.to_csv(f"{save_dir}/spn_dimensionality_acceleration.csv")
