...
node_vars = {"C": C, "k": k, "v_r": v_r, "v_t": v_t, "tau_u": 1/a, "b": b, "kappa": kappa, "g_i": g_i, "E_i": E_i,
             "g_e": g_e, "E_e": E_e, "s_e": s_e, "tau_s": tau_s, "v": v_t, "Delta": Delta, "tau_x": tau_x}
...
inp[int(start/dt):int(stop/dt)] += np.linspace(0, 0.02, num=int((stop-start)/dt))
backend = 'default'
solver = 'euler'
out_vars = ['r', 'v', 'x']
...
results = template.run(simulation_time=T, step_size=dt, backend=backend, solver=solver, sampling_step_size=dts,
                       inputs={f'p/{neuron_type}_op/alpha': inp},
