	...
	double precision x1, x2, se_1, se_2, sp_1, sp_2, sa_1, sa_2
	double precision alpha, omega, slope, t_on, t_off, po
	...
	! define forcing signal of STN
	se_1 = alpha/(1.0 + exp(-slope*(x1-cos(omega*t_on/2.0))))
	se_2 = alpha/(1.0 + exp(-slope*(-x1-cos(omega*t_on/2.0))))

    ! define forcing signal of GPe-p
	sp_1 = beta/(1.0 + exp(-slope*(x1-cos(omega*t_on/2.0 + po))))
	sp_2 = beta/(1.0 + exp(-slope*(-x1-cos(omega*t_on/2.0 + po))))

	! define forcing signal of GPe-a
	sa_1 = gamma/(1.0 + exp(-slope*(x1-cos(omega*t_on/2.0 + po))))
	sa_2 = gamma/(1.0 + exp(-slope*(-x1-cos(omega*t_on/2.0 + po))))
	...
	! at STN
	y_delta(8) = x_e
	y_delta(9) = (k_ee*r_ee + se_1 - se_2 - x_e*(tau_ampa_r+tau_ampa_d)
	 & - E_e) / (tau_ampa_r*tau_ampa_d)
	y_delta(10) = y_e
	...
	y_delta(14) = y_p
	y_delta(15) = (k_pp*r_xp + k_pa*r_xa + k_ps*r_s + sp_1 - sp_2
	...
	y_delta(17) = (k_ae*r_xe - x_a*(tau_ampa_r+tau_ampa_d) - E_a)
     & / (tau_ampa_r*tau_ampa_d)
	y_delta(18) = y_a
	y_delta(19) = (k_ap*r_xp + k_aa*r_xa + k_as*r_s + sa_1 - sa_2
    ...
    ! driver
	y_delta(38) = -omega*x2 + x1*(1-x1*x1-x2*x2)
	y_delta(39) = omega*x1 + x2*(1-x1*x1-x2*x2)
	...
	y(4) = -2.0
	y(6) = -4.0
	y(38) = cos(8.0*atan(1.0D0)*t)
	y(39) = sin(8.0*atan(1.0D0)*t)
