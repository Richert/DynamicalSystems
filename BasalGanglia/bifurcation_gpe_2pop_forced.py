...
store_params = ['PAR(20)', 'PAR(21)', 'PAR(23)', 'PAR(14)']
...
if any(c1):
    ...
    if c1[0]:
        ...
        c0_sols, c0_cont = a.run(starting_point=starting_point, origin=starting_cont, c='qif_lc', ICP=[20, 11],
                                 NPAR=n_params, name='c1:alpha', NDIM=n_dim, NMX=2000, DSMAX=0.05, RL0=alpha_min,
                                 RL1=alpha_max, STOP={}, UZR={20: [1.0]}, variables=store_vars, params=store_params)

        # step 2: codim 1 investigation of of inhibitory driver strength
        c1_sols, c1_cont = a.run(starting_point='UZ1', origin=c0_cont, c='qif_lc', ICP=[23, 11],
                                 NPAR=n_params, name='c1:omega', NDIM=n_dim, NMX=8000, DSMAX=0.05, RL0=omega_min,
                                 RL1=omega_max, STOP={}, UZR={23: [50.0]}, bidirectional=True, variables=store_vars,
                                 params=store_params)
        ...
        for s in c1_sols.values():
            if 'TR' in s['bifurcation']:
                i += 1
                p_tmp = f'TR{i}'
                c2_sols, c2_cont = a.run(starting_point=p_tmp, origin=c1_cont, c='qif3', ICP=[23, 20, 11],
                ...
                # if "R2" in bfs:
                #     s_tmp, c_tmp = a.run(starting_point='R21', origin=c2_cont, c='qif_lc', ICP=[23, 11],
        ...
        # c3_sols, c3_cont = a.run(starting_point='UZ1', origin=c0_cont, c='qif_lc', ICP=[23, 11],
