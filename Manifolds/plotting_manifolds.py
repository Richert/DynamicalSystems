...
# get filenames
path = 'results'
fns = [f'rnn_{i}.p' for i in range(20)]
...
# get example modules
idx = np.argmin(np.abs(deltas - 1.0))
...
ax2 = axes[1, 0]
ax2.bar(deltas, [m[0] for m in modules], yerr=[m[1] for m in modules], color='grey')
# ax2.set_xticks(np.arange(0, len(deltas), step=2), labels=[ps[idx] for idx in x[::2]])
