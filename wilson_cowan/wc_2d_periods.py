...
algorithm_params = {"NTST": 400, "NCOL": 4, "IAD": 3, "IPLT": 0, "NBC": 0, "NINT": 0, "NMX": 8000, "NPR": 5,
                    ...
                    "EPSS": 1e-9, "DSMIN": 1e-12, "DSMAX": 1e-2, "IADS": 1, "THL": {}, "THU": {}}
...
for i in range(n):
    res, s = ode.run(starting_point=f"UZ{i+1}", origin="s_e:ss", bidirectional=True, name=f"s_i:ss:{i+1}", STOP={"BP1"},
                     ICP="I/wc_i/s", IPS=1, ILP=0, ISP=2, ISW=1, RL0=-3.0, RL1=5.0, NPR=50, DS=1e-3, UZR={})
    if "HB" in res["bifurcation"].values:
        n_attempts = 0
        while n_attempts < 5:
            try:
                res2, _ = ode.run(starting_point="HB1", origin=s, name=f"s_i:lc:{i+1}", ICP="I/wc_i/s", ISW=-1, IPS=2,
                                  ISP=2, STOP={"BP1", "LP3"}, NPR=10, NMX=2000, get_period=True, variables=[], ILP=1,
