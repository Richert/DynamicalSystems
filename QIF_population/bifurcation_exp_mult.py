...
if codim1:
	...
	# analysis of chaotic behaviour at period doubling bifurcations
	chaos_analysis_pd = dict()

	# iterate over the names of all limit cycle continuations stored in pds
	for s in pds:
		
		# extract the solution curve for a given continuation
		...
		chaos_analysis_pd[s] = dict()
		chaos_analysis_pd[s]['eta'] = etas
		chaos_analysis_pd[s]['lyapunov_exponents'] = lyapunovs

		# compute fractal dimension of attractor at each solution point
		chaos_analysis_pd[s]['fractal_dim'] = [fractal_dimension(lyapunovs)]
...
if codim1:
	...
	kwargs['chaos_analysis_pd'] = chaos_analysis_pd
