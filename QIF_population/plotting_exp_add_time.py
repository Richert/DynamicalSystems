...
eta_rounded = list(map(lambda x : round(x, 5), chaos_data['eta']))
...
ax = a.plot_continuation('PAR(1)', 'U(1)', cont=f'eta_2', ax=f_ax1, default_size=markersize1)
...
for i, eta in enumerate(eta_rounded):
    ...
    #ax.set_title(fr'$\eta$ ={eta}'+f'\nMax LP: {max_LP}'+f'\nDim: {fract_dimen}', color=colors[i])


imagepath='results/'+f'bif_exp_add_time_{eta_rounded}'+'.pdf'
