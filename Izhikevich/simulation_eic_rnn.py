...
def ik_ei_ata(y: np.ndarray, N: int, inp: np.ndarray, ve_r: float, vi_r: float, ve_t: np.ndarray, vi_t: np.ndarray,
    ...
    se_ampa, se_gaba, si_ampa, si_gaba = y[m], y[m+1], y[m+2], y[m+3]
...
u_init[:N] -= 60.0
u_init[2*N:3*N] -= 60.0
model = RNN(N, 4*N+4, ik_ei_ata, Ce=Ce, Ci=Ci, ke=ke, ki=ki, ve_r=ve_r, vi_r=vi_r, ve_t=spike_thresholds_e,
